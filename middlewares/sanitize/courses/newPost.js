    ...
    if(typeof req.body.title !== 'undefined') 
    {
      if(!(titleRules.minLength <= req.body.title.length) || !(req.body.title.length <= titleRules.maxLength))  
      {
        if(titleRules.minLength > req.body.title.length) 
        {
          ...
          req.flash('validationErrors', 'Titre requis ou trop courts 6 caractères minimum.');
          // redirect
        ...
        {
          // create the error message with flash 
          req.flash('validationErrors', 'Titre trop longs 60 caractères maximum.');
          // redirect
          return res.redirect('/courses/new');
        }
      } 
      ...
      {
        // create the error message with flash 
        req.flash('validationErrors', 'Titre invalide | lettres, nombres, caractéres spéciaux sont autorisées.');
        // redirect
        return res.redirect('/courses/new');
      }
    }
