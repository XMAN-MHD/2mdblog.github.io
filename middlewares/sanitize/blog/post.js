...
const sanitize = async(req, res, next) => {
  ...
  /* 1- input vallidation */
    ...
    if(typeof req.body.title !== 'undefined') 
    {
      if(!(titleRules.minLength <= req.body.title.length) || !(req.body.title.length <= titleRules.maxLength))  
      {
        if(titleRules.minLength > req.body.title.length) 
        {
          ...
          req.flash('validationErrors', 'Titre requis ou trop courts minimum 6 caractères.');
          // redirect
        ...
        {
          // create the error message with flash 
          req.flash('validationErrors', 'Titre trop longs maximum 60 caractères.');
          // redirect
          return res.redirect('/posts/new');
        }
      } 
    ...
    else 
    {
      ...
      {
        // create the error message with flash 
        req.flash('validationErrors', "Type de fichier non valide | les formats JPEG, PNG et GIF sont autorisés.");
        //redirect
        return res.redirect('posts/new');
      }
